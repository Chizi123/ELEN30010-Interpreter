	...
	for (i = 0; fgets(line, 1024, in); i++) {
		...
		for (int j = 0; line[j]; j++) {
			...
			case 0: ins[i].cmd[cmdcnt++] = line[j];
				break;
			case 1: ins[i].addr1[ad1cnt++] = line[j];
				break;
			case 2: ins[i].addr2[ad2cnt++] = line[j];
				break;
			case 3: ins[i].addr3[ad3cnt++] = line[j];
				break;
			default:
				break;
			}
		}
		ins[i].cmd[cmdcnt] = '\0'; ins[i].addr1[ad1cnt] = '\0'; ins[i].addr2[ad2cnt] = '\0'; ins[i].addr3[ad3cnt] = '\0';
		if (cmdcnt == 3)
			ins[i].cmd[++cmdcnt] = '\0';
		if (cmdcnt == 4)
			ins[i].cmd[++cmdcnt] = '\0';
